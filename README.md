...
- Workflow разложены по стадиям: `validate -> build-release -> verify-target -> deploy`.
- `build-release` проверяет, что сайт собирается в publishable-директорию.
- `verify-target` отдельно проверяет SSH-доступ и права записи до запуска доставки.
